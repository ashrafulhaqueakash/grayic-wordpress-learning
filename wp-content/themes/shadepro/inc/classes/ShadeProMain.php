...
namespace ShadeProTheme\Inc\Classes;
...
class ShadeProMain
{
    ...
    public function postMarkupGenerator($layout, $grid)
    {
        ...
        if (!is_active_sidebar('shadepro_blog_sidebar')) :
            ...
            if (isset($layout)) :
                ...
                $this->thePostLoop('col-md-8', $grid);
    ...
    public function thePostLoop($template, $grid, $fullpage = false)
    {
        ...
        if ($fullpage == true) {
            echo '<div class="' . $template . ' post_col"> <div class="row">';
    ...
    public function pagination()
    {
        ...
        echo '<div class="col-12"><div class="shadepro-navigation">';
    ...
    public function shadepro_breadcrumb_bridge()
    {
        $shadepro = get_option('shadepro');

        if (isset($shadepro['breadcrumb_on'])) :
            if ($shadepro['breadcrumb_on'] == true) :
                $this->shadepro_get_the_breadcrumbs();
            endif;
        else :
            $this->shadepro_get_the_breadcrumbs();
        endif;
     ...
     * @return breadcrumb
     */
    public function shadepro_get_the_breadcrumbs()
    {
        $shadepro = get_option('shadepro');
        ...
        $blog_hero_caption = isset($shadepro['blog_hero_caption']) ? $shadepro['blog_hero_caption'] : 'Read our latest blog posts';
        $cs_hero_caption = isset($shadepro['cs_hero_caption']) ? $shadepro['cs_hero_caption'] : '';
        $job_hero_caption = isset($shadepro['job_hero_caption']) ? $shadepro['job_hero_caption'] : 'Join at Grayic';
        $shop_bread_caption = isset($shadepro['shop_bread_caption']) ? $shadepro['shop_bread_caption'] : '';
        ...
        if ('case-study' == shadepro_get_archive_post_type()) {
        ...
        } elseif ('job' == shadepro_get_archive_post_type() || is_tax( 'job-category' )|| is_tax( 'job-location' )|| is_tax( 'job-address' )) {
    ...
    public function generateBreadCrumbTitle()
    {
        ...
        if (is_home() || is_front_page()) {
            $title = isset($shadepro['bp_title']) ? esc_html($shadepro['bp_title']) : esc_html__('Our Blog', 'shadepro');
        } elseif ('case-study' == shadepro_get_archive_post_type()) {
            $title = isset($shadepro['cs_title']) ? esc_html($shadepro['cs_title']) : esc_html__('Case Study', 'shadepro');
        } elseif ('job' == shadepro_get_archive_post_type()) {
            $title = isset($shadepro['job_title']) ? esc_html($shadepro['job_title']) : esc_html__(' Current job openings', 'shadepro');
        } elseif (is_page()) {
            $title = get_the_title();
        } elseif (is_single()) {
            $title = get_the_title();
        } elseif (function_exists('is_shop') && is_shop()) {
            $title = isset($shadepro['shop_title']) ? esc_html($shadepro['shop_title']) : esc_html__('Our Products', 'shadepro');
        } elseif (is_archive()) {
            $title = get_the_archive_title();
        } elseif (is_search()) {
            $title = esc_html__('Search results for: ', 'shadepro') . get_search_query();
        }

        return $title;
    }

    public static function shadepro_breadcrumbs()
    {

        $shadepro = get_option('shadepro');

        $sepOpt = (isset($shadepro['breadcrumb_sep']) ? $shadepro['breadcrumb_sep'] : '<i class="fa fa-angle-right"></i>');

        /* === OPTIONS === */
        $text['home'] = esc_html__('Home', 'shadepro'); // text for the 'Home' link
        $text['category'] = esc_html__('Archive by Category "%s"', 'shadepro'); // text for a category page
        $text['search'] = esc_html__('Search Results for "%s" Query', 'shadepro'); // text for a search results page
        $text['tag'] = esc_html__('Posts Tagged "%s"', 'shadepro'); // text for a tag page
        $text['author'] = esc_html__('Articles Posted by %s', 'shadepro'); // text for an author page
        $text['404'] = esc_html__('Error 404', 'shadepro'); // text for the 404 page
        $text['page'] = esc_html__('Page %s', 'shadepro'); // text 'Page N'
        $text['cpage'] = esc_html__('Comment Page %s', 'shadepro'); // text 'Comment Page N'
